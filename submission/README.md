...
This directory is the canonical archive for the submitted **Maybe Tomorrow.**
OpenAI Build Week entry. The application, public demo, Devpost page, and the
creation process remain reproducible evidence for judges.
...
- [x] Untested screen-reader platforms remain explicitly **Pending** in the
      accessibility matrix; no accessibility certification is claimed.
...
- [x] Yoshie watched and listened to the complete rights-cleared English
      ...
      balance, captions, and final visual/rights pass.
- [x] Yoshie uploaded the video as **Public** and signed-out playback passed.
- [x] Yoshie ran `/feedback` in the primary Codex build task; the private
      Session ID is not stored here.
- [x] Yoshie joined Devpost, confirmed the account-only eligibility and rights
      statements, completed every required field, reviewed the public page,
      and submitted.
...
1. The reviewed submission PR was merged.
2. The post-merge GitHub Pages deployment succeeded.
3. Both exact demo paths passed in a clean browser session.
4. The completed English-master results are preserved in
   [`video/QA_CHECKLIST.md`](./video/QA_CHECKLIST.md); none were carried forward
   from the rejected System Voice candidate.
5. YouTube Studio showed **Public**, and signed-out playback passed.
6. The repository remained public with `LICENSE` and
   `THIRD_PARTY_NOTICES.md`.
7. `/feedback` was completed in the primary Codex build task, and its Session
   ID remains outside the repository.
8. The Devpost form and public project page were reviewed before final Submit.
